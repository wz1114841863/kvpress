...
A4124_SCHEMA = "kvzap-route-a4124-multitoken-bridge-gate-1.0"
...
def main():
    ...
    diagnostic={"context_token_count":int(context_ids.shape[1]),"question_token_count":int(question_ids.shape[1]),"full_kv_bypass":logit_summary(full,top_k=a.top_k),"same_mask_dense_replay":{"logits":logit_summary(dense,top_k=a.top_k),"policy_decode_calls":dense_backend.policy_decode_calls,"replay_consumption":dense_backend.replay_consumption_summary()},"same_mask_route_a_owned_cold_replay":{"logits":logit_summary(route,top_k=a.top_k),"policy_decode_calls":route_backend.policy_decode_calls,"policy_multi_token_calls":route_backend.policy_multi_token_calls,"policy_multi_token_tokens":route_backend.policy_multi_token_tokens,"replay_consumption":route_backend.replay_consumption_summary(),"coverage":route_backend.coverage(),"native_cold_ownership":route_backend.ownership_summary()},"full_vs_dense":paired_logit_relation(full,dense),"dense_vs_route":relation}
    m={"schema_version":A4124_SCHEMA,"status":"complete","created_at":datetime.now(timezone.utc).isoformat(),"git_commit":get_git_commit(),"config":config,"config_hash":stable_hash(config),"request_id":req["request_id"],"request_content_hash":stable_hash({"context":req["context"],"question":req["question"]}),"replay_source":{"directory":str(a.replay_source_dir),"event_file_sha256":digest,"source_manifest_sha256":sha256_file(a.replay_source_dir/"a41_replay_mask_source_manifest.json"),"event_count":source["event_count"]},"diagnostic":diagnostic,"observational_guards":{"prefix_replay_only":True,"causal_multitoken_route_a_bridge_complete":True,"finite_same_mask_dense_and_route_logits":True,"same_mask_dense_route_first_argmax_equal":True,"native_dense_cold_slots_physically_freed":False},"boundaries":["Semantic prefix gate only; not timing or physical storage evidence.","Safe native placeholders protect selected head values only while native attention computes unselected heads.","No greedy decode or complete replay claim."],"torch_version":str(torch.__version__),"transformers_version":str(transformers.__version__)}
