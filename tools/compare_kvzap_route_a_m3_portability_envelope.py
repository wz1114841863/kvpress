...
M3_SCHEMA = "kvzap-route-a-m3-portability-envelope-comparison-1.0"
...
def main() -> None:
    ...
    if int(lstructure["query_heads_per_kv_head_group"]) != 4 or qwen["qwen3_8b_kvzap_resource_descriptor"]["qwen_specific_group_width_values"] != [4]:
    ...
    report = {
        ...
        "separate_fixed_workload_descriptors": {
            "qwen3_8b_kvzap": {
                "model": qscope["model"], "layer_count": int(qscope["observed_layer_count"]), "kv_head_count": int(qscope["observed_kv_head_count"]),
                "workloads": qwen["qwen3_8b_kvzap_resource_descriptor"]["workload_descriptor"],
                "interpretation": "Three fixed Qwen requests; source/fan-in/page-tail distributions remain Qwen-specific.",
            },
            "nous_llama31_8b_kvzap": {
                "model": m0["config"]["model_repo_id"], "layer_count": int(lstructure["layer_count"]), "kv_head_count": int(lstructure["kv_head_count"]),
        ...
        "comparison_boundaries": {
            ...
            "not_directly_comparable": ["Qwen has three fixed workloads while Llama has one fixed request.", "Qwen descriptor source/fan-in fractions and page/tail distributions are not a distribution for Llama.", "Llama final-state maxima are not Qwen-equivalent event distributions or a universal resource bound.", "Different layer/KV-head totals (Qwen 36/288 versus Llama 32/256) are workload/model descriptors, not accelerator dimensions."],
        ...
        "observational_guards": {"all_input_hashes_and_required_guards_verified": True, "qwen_and_llama_descriptors_kept_separate": True, "two_model_shared_semantic_contract_covered": True, "no_hardware_parameter_selected": True, "no_model_runtime_loaded": True},
        "boundaries": ["M3 is a hash-bound comparison of already-completed artifacts. It does not execute a model, estimate capacity/traffic/latency, or select any hardware parameter.", "Shared page/window/GQA values are observed reference compatibility fields, not a universal design point.", "This report is not an architecture specification, hardware measurement, or RTL gate."],
    }
