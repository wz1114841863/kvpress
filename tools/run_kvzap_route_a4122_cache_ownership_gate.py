...
A4122_SCHEMA = "kvzap-route-a4122-cache-ownership-gate-1.0"
...
def main() -> None:
    ...
    if dense_answer != route_answer or dense_tokens != route_tokens:
        raise AssertionError("same-mask dense control and selected-head ownership Route-A path produced different generated output")
    ...
    manifest = {"schema_version": A4122_SCHEMA, "status": "complete", "created_at": datetime.now(timezone.utc).isoformat(), "git_commit": get_git_commit(), "config": config, "config_hash": stable_hash(config), "request_id": request["request_id"], "request_content_hash": stable_hash({"context": request["context"], "question": request["question"]}), "replay_source": {"directory": str(args.replay_source_dir), "event_file_sha256": event_sha256, "source_manifest_sha256": sha256_file(args.replay_source_dir / "a41_replay_mask_source_manifest.json"), "event_count": source["event_count"], "source_answer_sha256": source["answer_sha256"]}, "outcomes": outcomes, "observational_guards": {"paired_mask_mode": "replayed_dense_mask", "full_kv_bypass_zero_route_a_admission": True, "route_a_predictor_scored_online": False, "replay_mask_consumption_complete": True, "same_mask_dense_route_answer_and_token_ids_equal": True, "selected_native_mature_cold_values_poisoned": True, "selected_native_cold_read_guard_complete": True, "native_dense_cold_slots_physically_freed": False}, "boundaries": ["This is an A4.1.2.2 semantic ownership gate, not timing, throughput, allocator, HBM traffic, energy, area, frequency, hardware acceleration, or RTL evidence.", "NaN poisoning proves selected mature original K/V is not silently consumed from the native dense cache by the selected Route-A attention path; it does not remove the native dense tensor allocation.", "Full-KV bypass is a policy control. Same-mask dense and Route-A owned-cold output equality is required; Full-KV equality is recorded, not required."], "torch_version": str(torch.__version__), "transformers_version": str(transformers.__version__)}
