...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="enforce",
        help="Keep the default enforce guard, or retain bounded ULP-breach diagnostics without accepting them as a strict pass.",
    )
    parser.add_argument(
        "--ulp-breach-sample-limit",
        type=int,
        default=32,
        help="Bounded scalar diagnostic sample count when --execution-dtype-ulp-mode=record_only.",
    )
...
def run_point(*, name: str, budget: int, pipe, request: dict[str, Any], args: argparse.Namespace, revision: str, layers: int, heads: int) -> dict[str, Any]:
    ...
    return {"admission_budget": budget, "online_same_mask_dense": {"answer_sha256": answer_hash(dense), "policy_decode_call_count_by_layer": dense_backend.policy_decode_calls, "policy_coverage": dense_coverage, "same_mask_numerical_guard_work": dense_backend.same_mask_numerical_guard_work_summary(), "execution_dtype_ulp_breach_summary": dense_backend.execution_dtype_ulp_breach_summary()}, "replayed_same_mask_route_a": {"answer_sha256": answer_hash(route), "policy_decode_call_count_by_layer": route_backend.policy_decode_calls, "policy_coverage": coverage, "same_mask_numerical_guard_work": route_backend.same_mask_numerical_guard_work_summary(), "execution_dtype_ulp_breach_summary": route_backend.execution_dtype_ulp_breach_summary(), "source_coverage": sources, "final_lifecycle_state": final_states(route_backend, layers=layers, heads=heads), "page_witness": page_witness(coverage)}, "same_mask_pairing": {"mode": "replayed_online_dense_mask", "route_a_replay_consumption_complete": True, "original_mask_decision_count_by_layer": {str(row["layer"]): int(row["original_mask_decision_count"]) for row in dense_coverage["layers"]}}}
...
def main() -> None:
    ...
    if min(args.context_repetitions, args.page_tokens, args.max_new_tokens, args.max_executed_dtype_ulps, args.ulp_breach_sample_limit) <= 0 or args.max_new_tokens < 2:
