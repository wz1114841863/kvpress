...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prefill-maturity-chunk-tokens", type=int, default=0, help="A4.3.4 trace-on-only prefill micro-event size. Zero retains one batch maturity append; positive values require --record-lifecycle-transitions and do not change the default path.")
...
def main() -> None:
    ...
    if args.prefill_maturity_chunk_tokens < 0:
        raise ValueError("--prefill-maturity-chunk-tokens must be non-negative")
    if args.prefill_maturity_chunk_tokens and not args.record_lifecycle_transitions:
        raise ValueError("--prefill-maturity-chunk-tokens requires --record-lifecycle-transitions")
    ...
    if args.record_lifecycle_transitions:
        ...
        traced_backend = RouteAPolicyAttentionBackendSet(pipe.model, None, layers=selected_layers, kv_head=selected, threshold=args.threshold, window=args.window_size, page_tokens=args.page_tokens, admission_budget=args.admission_budget, rtol=args.rtol, atol=args.atol, max_executed_dtype_ulps=args.max_executed_dtype_ulps, execution_dtype_ulp_mode=args.execution_dtype_ulp_mode, execution_dtype_close_mode=args.execution_dtype_close_mode, ulp_breach_sample_limit=args.ulp_breach_sample_limit, replay_mask_events=replay_events, lifecycle_transition_recorder=transition_recorder, prefill_maturity_chunk_tokens=args.prefill_maturity_chunk_tokens)
        trace_mode = "prefill micro-event" if args.prefill_maturity_chunk_tokens else "lifecycle-transition"
        print(f"Pass {total_passes}/{total_passes}: trace-on Route-A {trace_mode} replay in layers {list(selected_layers)}...")
    ...
    config = {key: value for key, value in vars(args).items() if key not in {"output_dir", "gate_a_evidence"}}
    manifest = {
        "schema_version": "kvzap-route-a40-policy-on-qwen-gate-1.7" if args.prefill_maturity_chunk_tokens else ("kvzap-route-a40-policy-on-qwen-gate-1.6" if args.record_lifecycle_transitions else "kvzap-route-a40-policy-on-qwen-gate-1.5"), "created_at": datetime.now(timezone.utc).isoformat(), "git_commit": get_git_commit(),
