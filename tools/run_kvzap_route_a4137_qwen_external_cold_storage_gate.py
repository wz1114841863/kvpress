"""A4.1.3.2 untimed Qwen selected-head external-cold interface gate.

The selected Qwen attention hook uses the explicit external Route-A adapter
for all mature selected-head reads. Native DynamicCache cold values are still
NaN-poisoned, so a hidden fallback is observable. This does not yet replace or
shrink DynamicCache itself.
"""
...
A4137_SCHEMA = "kvzap-route-a4137-qwen-external-cold-storage-gate-1.0"


def run_path(*, pipe, context_ids: torch.Tensor, question_ids: torch.Tensor, backend, args: Any) -> tuple[str, list[int]]:
    ...
    if not isinstance(result, tuple) or len(result) != 2 or not isinstance(result[0], str) or not isinstance(result[1], list) or not result[1]:
        raise AssertionError("A4.1.3.2 gate did not return an answer plus nonempty generated token IDs")
    return result


def require_external_interface(*, backend: RouteAQwenExternalColdStorageAttentionBackend, args: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if backend.policy_decode_calls <= 0 or not backend.comparisons:
        raise AssertionError("A4.1.3.2 observed no selected-head Route-A decode attention")
    ...
    if not external["qwen_external_cold_storage_interface_active"]:
        raise AssertionError("Qwen external cold-storage interface was not active")
    if external["adapter_selected_native_cold_tensor_tokens"] != 0:
        raise AssertionError("external adapter retained selected mature cold tensors")
    if external["transformers_dynamic_cache_substitution"]:
        raise AssertionError("A4.1.3.2 must not claim DynamicCache substitution")
    return coverage, ownership


def main() -> None:
    args = parse_args(description="A4.1.3.2 untimed Qwen selected-head external-cold storage interface gate; not an allocator or performance benchmark.")
    ...
    if args.target_layer < 0 or args.target_kv_head < 0 or min(args.context_repetitions, args.page_tokens, args.admission_budget, args.max_new_tokens, args.max_executed_dtype_ulps) <= 0 or args.window_size < 0:
        raise ValueError("invalid A4.1.3.2 dimensions")
    ...
    if (args.model_name, args.predictor_name, args.model_revision, args.predictor_revision) != (DEFAULT_MODEL, DEFAULT_PREDICTOR, GATE_B_MODEL_REVISION, GATE_A_PREDICTOR_REVISION):
        raise ValueError("A4.1.3.2 is bounded to frozen Qwen3-8B and official MLP revisions")
    ...
    initialize_output_directory(
        args.output_dir, config=config, git_commit=get_git_commit(), record_name="a4137_qwen_external_cold_storage_started.json", schema_version=A4137_SCHEMA,
        boundaries=[
            "A4.1.3.2 is an untimed Qwen semantic integration gate, not a latency or allocator measurement.",
            "Selected mature native DynamicCache K/V is NaN-poisoned while Qwen selected attention reads the external adapter's Route-A hot/pending/packed state.",
            "Native DynamicCache allocation and dense shape remain present; external adapter storage is not a DynamicCache replacement and does not establish physical memory savings.",
        ],
    )
    common = dict(layer=args.target_layer, kv_head=args.target_kv_head, threshold=args.threshold, window=args.window_size, page_tokens=args.page_tokens, admission_budget=args.admission_budget, rtol=args.rtol, atol=args.atol, max_executed_dtype_ulps=args.max_executed_dtype_ulps, replay_mask_events=events[args.target_layer])
    ...
    print("Pass 3/3: Qwen Route-A external selected-head cold-storage interface...")
    ...
    assert_no_runtime_mask_state(pipe.model)
    route_coverage, ownership = require_external_interface(backend=route_backend, args=args)
    relation = generated_output_relation(dense_answer, dense_tokens, route_answer, route_tokens)
    manifest = {
        "schema_version": A4137_SCHEMA, "status": "complete", "created_at": datetime.now(timezone.utc).isoformat(), "git_commit": get_git_commit(),
        "config": config, "config_hash": stable_hash(config), "request_id": request["request_id"],
        ...
        "outcomes": {
            ...
            "same_mask_route_a_qwen_external_cold_replay": {"answer_sha256": answer_hash(route_answer), "generated_token_count": len(route_tokens), "generated_token_ids_sha256": token_ids_hash(route_tokens), "policy_decode_calls": route_backend.policy_decode_calls, "coverage": route_coverage, "native_cold_ownership": ownership},
        },
        "same_mask_dense_route_generated_output_relation": relation,
        "observational_guards": {
            "paired_mask_mode": "replayed_dense_mask", "full_kv_bypass_zero_route_a_admission": True, "route_a_predictor_scored_online": False,
            "replay_mask_consumption_complete": True, "fp32_same_mask_guard": {"rtol": args.rtol, "atol": args.atol},
            "selected_native_mature_cold_values_poisoned": True, "selected_native_cold_read_guard_complete": True,
            "qwen_external_cold_storage_interface_active": True, "adapter_selected_native_cold_tensor_tokens": 0,
            "transformers_dynamic_cache_substitution": False, "native_dense_cold_slots_physically_freed": False,
        },
        "boundaries": [
            "Untimed Qwen selected-head semantic integration only; this is not timing, throughput, allocator, HBM traffic, energy, area, frequency, hardware acceleration, or RTL evidence.",
            "The external adapter supplies selected mature retained K/V through Route-A state and has no selected mature cold tensor. Native DynamicCache remains allocated and is only poisoned as a no-fallback guard.",
            "Same-mask dense/Route-A generated output relation is recorded, not required. Guarded reduction differences can alter later greedy tokens.",
        ],
        "torch_version": str(torch.__version__), "transformers_version": str(transformers.__version__),
    }
    path = args.output_dir / "a4137_qwen_external_cold_storage_manifest.json"
    path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"A4.1.3.2 Qwen external cold-storage interface gate passed: {path}")
