...
class KVzapPress(ScorerPress):
    ...
    predictor_repo_id_override: Optional[str] = None
    kvzap_model_name: Optional[str] = field(default=None, init=False)

    def post_init_from_model(self, model):
        derived_model_name = f"nvidia/KVzap-{self.model_type}-{model.config.name_or_path.split('/')[-1]}"
        kvzap_model_name = self.predictor_repo_id_override or derived_model_name
