...
The backend is deliberately narrow: batch one, a selected layer/KV head, and
...
class RouteAPolicyAttentionBackend(AbstractContextManager):
    """Attach one real policy-on head-group without fake-key masking."""

    def __init__(self, model, predictor, *, layer: int, kv_head: int, threshold: float, window: int, page_tokens: int, admission_budget: int, rtol: float, atol: float) -> None:
    ...
    def _append_state(self, key: torch.Tensor, value: torch.Tensor) -> None:
        ...
        if self.state is None:
            if not 0 <= self.kv_head < key.shape[1]:
    ...
    def attention(self, original: Callable[..., Any], module, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, attention_mask: torch.Tensor | None, dropout: float, **kwargs: Any):
        ...
        if self.kv_head >= kv_heads:
            raise AssertionError("target KV head changed after backend initialization")
        ...
        for query_head in range(heads):
            ...
            if mapped_kv_head == self.kv_head:
                route = self.state.attention(q * scaling, head=self.kv_head).to(dtype=q.dtype)
                dense = dense_same_mask_attention(q * scaling, self.state.same_mask_records(self.kv_head)).to(dtype=q.dtype)
        ...
        if route is None or dense is None:
        ...
        selected = self.state.state_summary(self.kv_head)
        selected["cache_position"] = key.shape[2] - 1
        selected["max_abs_difference"] = float((route - dense).abs().max().item())
